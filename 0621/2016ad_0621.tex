...
\subsection{クイックソート}
アルゴリズムの分野の代表的な問題として整列問題がある．

\vspace{1em}
\noindent
入力: 整数の配列 $A$.\\
問題: 整数の配列 $A$ を小さい順に並べたものを出力せよ．

\vspace{1em}
\noindent
クイックソートはこの問題を解く簡単なアルゴリズムである．
...
\item クイックセレクトとクイックソートの計算量のオーダーをもとめよ．ただしピボットとして中央値を比較0回で選択すると仮定せよ．
\item クイックセレクトとクイックソートの計算量のオーダーをもとめよ．ただしピボットとして最小値を比較0回で選択すると仮定せよ．
...
ランダムな入力に対するクイックセレクト，クイックソートの計算量は高い確率で上の課題の3でもとめたものになる．このようにランダムな入力に対して計算量の平均をとったものを\textbf{平均計算量}と呼ぶ．
